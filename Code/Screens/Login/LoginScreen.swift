...
struct LoginScreen: View {
    ...
    var body: some View {
        ...
        .onChange(of: inputText, perform: suggestAutoCompleteResults)
        .onAppear {
            Analytics.open(screen: .login)
            ErrorReporting.breadcrumb(.loginScreen)
            Task {
    ...
    private func showError() {
        bannerController.show(
            ...
            position: .bottom,
            actions: [
                .destructive(title: Localized.Action.createNewCodeAccount) {
                    isActive = false
                },
...
struct LoginScreen_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView {
            LoginScreen(isActive: .constant(true))
        }
