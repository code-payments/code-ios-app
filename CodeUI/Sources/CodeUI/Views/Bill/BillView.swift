...
public struct BillView: View {
    ...
    /// Code:      0.555
    ///
    public init(kin: Kin, data: Data, canvasSize: CGSize, aspectRatio: CGFloat = 0.555, action: VoidAction? = nil) {
        self.kin    = kin
        self.data   = data
        self.canvasSize = canvasSize
        self.action = action ?? {}
        self.string = kin.formattedTruncatedKin()
    ...
    static var previews: some View {
        Group {
            ForEach(sizes, id: \.width) { size in
                BillView(kin: 5_000_000_000, data: .placeholder35, canvasSize: size)
