...
| Reading `supplyFromBonding` from a `StoredBalance` snapshot for intent math | **For any value that must match the attached verified proof, read live from `RatesController`, not from `selectedBalance.stored.X`.** `StoredBalance` fields (`supplyFromBonding`, etc.) are a captured struct loaded from SQLite at screen-open and do NOT refresh when the stream tick updates the actor cache — even though the DB row gets rewritten. `rate` was already live via `ratesController.rateForEntryCurrency()`; `supply` is live via `ratesController.supplyFromBonding(for: mint)`. Using the snapshot for on-wire `(quarks, nativeAmount)` diverges from the proof `SendCashOperation` attaches and the server rejects with `invalidIntent(native amount does not match expected sell value)`. General rule: if the server will re-validate a value against a signed proto, the client must read that value from the same live source (`RatesController`'s `@Observable` caches mirror the `VerifiedProtoService` actor), never from a captured struct field. |
