...
5. **VerifiedState** - Bundles server-signed exchange rate proof (`rateProto`) and optional reserve state proof (`reserveProto`). Required when submitting any payment intent. For **launchpad currencies**, `reserveProto` is mandatory — the server rejects intents without it. **Pin-at-compute invariant**: the rate used to compute `ExchangedFiat.quarks` must match the rate the server validates the intent against (`rateProto`). Amount-entry flows (`CurrencyBuyViewModel`, `CurrencySellViewModel`, `WithdrawViewModel`) fetch a pin at the commit moment via `prepareSubmission()` and compute once against that pin — fetching twice per submit, or pinning at flow-open and holding across entry, reintroduces the "native amount and quark value mismatch" server reject.
