...
| Canceling/modifying `SendCashOperation` in `Session.Cash.dismissBill` | **Never** explicitly call `cancel()` or `invalidateMessageStream()` on `SendCashOperation` from `dismissBill`. After a grab, the received bill is a **live** `SendCashOperation` that others can scan ("quick give and grab" chain). Setting `sendOperation = nil` is fine (deinit cleans up), but explicit teardown kills a live bill. The operation's `complete()` method handles stream teardown on success/failure. |
...
| Showing a received bill without `verifiedState` | Every call to `Session.Cash.showBill` must pass `verifiedState` — even for `received: true` bills. The received bill creates a live `SendCashOperation` for the "quick give and grab" chain. Without `verifiedState`, launchpad currency transfers fail with "reserve state is required". Both `Session.Cash.receive` (scan) and `receiveLink` (deep link) must provide it. |
...
- Flipcash/Core/Session/Session+Purchases.swift (buy/sell/launch, session.purchases)
- Flipcash/Core/Session/Session+Cash.swift (cash bills + links, session.cash)
