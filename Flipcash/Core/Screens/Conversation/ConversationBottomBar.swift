...
/// The curve the bar grows and shrinks on around the reply strip. The only thing a reply animates:
/// the surface behind the bar is permanent, so there is nothing to fade in or out over it.
///
...
/// The bar's own surface: one flat, opaque slab under the quote and the controls, square-edged and
/// full-bleed, running past the safe area to the bottom of the display.
///
...
/// changes two things about it — how tall it is, and what colour it is — and both resolve on the one
/// spring, so what moves is the bar itself rather than something arriving over the messages.
...
///
/// At rest it is the chat background: the slab is there, full width and opaque, but indistinguishable
/// from the transcript behind it, so the composer looks the way it always has. Replying lifts it to
/// the elevated-surface token, which is what draws the top edge and sets the quote apart from the
/// messages above it. Because the slab never appears or disappears, this is a colour change on a
/// stationary object rather than a panel arriving.
///
...
enum BarSurface {
    ...
    static func fill(isReplying: Bool) -> Color {
        isReplying ? .backgroundSecondary : .backgroundMain
    }
}
