...
struct AddMoneyStartScreen: View {
    ...
    ///
    /// The flow pushes onto the navigation stack the picker was launched from —
    /// matching the rest of the app — via `startFlow`, and a debit-card
    /// (Coinbase) deposit verifies phone/email first so no empty screen appears
    /// ahead of it (skipped over the buy sheet, already gated).
