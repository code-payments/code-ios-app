...
private struct WalletScreenContent: View {
    ...
    /// The card in flight between the deck and the page's hero slot.
    @State private var flyingCard: TokenCardData?
    /// Where that card currently sits, measured from the top of the container.
    @State private var flyingCardOffset: CGFloat = 0
    ...
    /// True once it has landed, at which point the page draws the card instead.
    @State private var cardHasLanded = false
    /// Whether the page has been brought in behind the flying card.
    @State private var pageIsIn = false
    /// Whether the page exists yet. Split from ``pageIsIn`` so the cost of
        ...
        NavigationStack(path: $router[.balance]) {
            ...
            .overlay {
                ...
                if pageIsBuilt, let expandingMint {
                    CurrencyInfoScreen(
                        mint: expandingMint,
                        presentation: .overlay(onClose: closeCard, showsHeroCard: cardHasLanded),
                        defersHeavyContent: !heavyContentReady
                    )
                    .id(expandingMint)
                    // Fades in place: sliding the page up drags its chrome and
                    // tiles in from the bottom. Safe to fade now that the deck is
                    // already gone, so nothing shows through it.
                    .background(Color.backgroundMain)
                    .opacity(pageIsIn ? 1 : 0)
                    // Not until the card is the page's own, rather than the copy
                    // still flying above it. The page finishes fading a moment
                    // before the spring is fully removed, and scrolling in that
                    // gap slides the content out from under a card that is not
                    // in the scroll view yet.
                    .allowsHitTesting(pageIsIn && cardHasLanded)
                }
            }
            // The opened card flies above the incoming page, so there is one card
            // moving and one continuous motion rather than two phases.
            .overlay(alignment: .top) {
                if let card = flyingCard {
                    TokenCardView(data: card, height: WalletCardGeometry.cardHeight)
                        .padding(.horizontal, 20)
                        .offset(y: flyingCardOffset)
                }
            }
            ...
            .onChange(of: router[.balance].count) { _, depth in
                guard depth == 0, expandingMint != nil else { return }
                closeCard()
            }
        }
    ...
    private func openCard(_ item: TokenCardData, currentTop: CGFloat) {
        ...
        withTransaction(seed) {
            flyingCard = item
            flyingCardOffset = currentTop - safeArea.top
            cardHasLanded = false
            pageIsIn = false
            pageIsBuilt = false
            expansionProgress = 0
        ...
        withAnimation(.smooth(duration: Self.liftDuration), completionCriteria: .removed) {
            flyingCardOffset = WalletCardGeometry.openCardTopInset
            expansionProgress = 1
            deckOpacity = 0
        } completion: {
            guard token == transitionToken else { return }
            // Both cards are in exactly the same place by now, so this swaps
            // them in one step and needs no overlap to cover a gap.
