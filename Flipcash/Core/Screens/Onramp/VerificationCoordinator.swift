...
final class VerificationCoordinator {
    ...
    private(set) var currentInlineViewModel: OnrampVerification?
    ...
    /// `.sheet(item:)` to this.
    var fallbackViewModel: OnrampVerification?
    ...
    /// `OnrampVerification`, hands it to `bind` so the caller can mount
    ...
    func runGated(
        ...
        if let profile = session.profile,
           ...
           CoinbaseOrderEmail.resolve(profile: profile) != nil {
            perform()
            return
        }
    ...
    func endInlineFlow(_ vm: OnrampVerification) {
