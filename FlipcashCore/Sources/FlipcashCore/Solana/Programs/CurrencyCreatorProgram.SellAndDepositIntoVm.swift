...
extension CurrencyCreatorProgram {
    ...
    /// 4. `[]` Base mint (USDC)
    /// 5. `[writable]` Vault target (mint of source token being sold)
    /// 6. `[writable]` Vault base (source of source tokens)
    /// 7. `[writable]` Seller target (temporary ATA of the source token)
    ...
    public struct SellAndDepositIntoVm: Equatable, Hashable, Codable {
        ...
        init(amount: UInt64, minOutAmount: UInt64, vmMemoryIndex: UInt16, seller: PublicKey, pool: PublicKey, currency: PublicKey, targetMint: PublicKey, baseMint: PublicKey, vaultTarget: PublicKey, vaultBase: PublicKey, sellerTarget: PublicKey, feeTarget: PublicKey, feeBase: PublicKey, vmAuthority: PublicKey, vm: PublicKey, vmMemory: PublicKey, vmOmnibus: PublicKey, vtaOwner: PublicKey) {
            self.seller = seller
            self.pool = pool
...
extension CurrencyCreatorProgram.SellAndDepositIntoVm: InstructionType {
    ...
    public init(instruction: Instruction) throws {
        let data = try CurrencyCreatorProgram.parse(.sellAndDepositIntoVm, instruction: instruction, expectingAccounts: 17)
    ...
    public func encode() -> Data {
        ...
        data.append(contentsOf: minOutAmount.bytes) // 8
        data.append(contentsOf: vmMemoryIndex.bytes) // 2
