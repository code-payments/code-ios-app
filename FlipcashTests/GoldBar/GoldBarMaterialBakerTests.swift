...
struct GoldBarMaterialBakerTests {
    ...
    func bakedCode_darkensAlbedo() throws {
        ...
        let textures = GoldBarMaterialBaker.bake(.full(code: code, stampLines: ["$25.00"], serial: PublicKey.usdf.base58))
    ...
    func bakedTextures_matchRequestedSize() {
        let code = GoldBarCodeRenderer.image(for: .placeholder35, side: 480)
        let config = GoldBarMaterialBaker.Config.full(code: code, stampLines: ["$25.00"], serial: PublicKey.usdf.base58)
