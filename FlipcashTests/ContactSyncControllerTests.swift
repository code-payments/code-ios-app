...
struct ContactSyncControllerTests {
    ...
    @Suite("removeContact(withE164:)")
    @MainActor
    struct RemoveContactTests {

        @Test("Drops the e164 from both DB tables")
        func dropsFromBothTables() throws {
            ...
            controller.removeContact(withE164: target)
        ...
        @Test("Triggers a sync after dropping")
        func triggersSync() {
            let controller = ContactSyncController(
                client:                      MockContactSync(),
                database:                    .mock,
            ...
            controller.removeContact(withE164: "+15551234567")

            // `sync()` flips `isSyncing` and assigns `syncTask` synchronously
            // before spawning the detached worker — both must be set together
            // for the trigger to have fired.
            #expect(controller.isSyncing == true)
            #expect(controller.syncTask != nil)
        }
    }
