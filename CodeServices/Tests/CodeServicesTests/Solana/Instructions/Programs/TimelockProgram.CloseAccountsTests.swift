//
//  TimelockProgram.CloseAccountsTests.swift
//  CodeServicesTests
//
...
class TimelockProgram_CloseAccountsTests: XCTestCase {
    
    func testDecode() throws {
        let (transaction, _) = SolanaTransaction.mockCloseDormantAccount()
        let rawInstruction = transaction.message.instructions[5]
        let instruction = try TimelockProgram.CloseAccounts(instruction: rawInstruction)
