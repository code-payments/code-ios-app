...
/// Tests created to validate key generation and deterministic
/// wallet generation using this tool: https://iancoleman.io/bip39/
///
class KeyTests: XCTestCase {
    
    // MARK: - Generate 16 -
    
    func testGenerate16ByteEntropy() throws {
        let entropy = try XCTUnwrap(Seed16.generate())
        XCTAssertEqual(entropy.bytes.count, 16)
    }
    
    func testGenerate12Words() throws {
        let entropy = try XCTUnwrap(Seed16.generate())
        let phrase = entropy.mnemonic
        
        XCTAssertEqual(phrase.words.count, 12)
    }
    
    func testGenerateSeedFrom12Words() {
        let entropy = Seed16(base58: "HqBqWa8sc9pkTGCbNTsiwH")!
        let seed = Derive.seedUsingBIP39(phrase: entropy.mnemonic.words)
        XCTAssertEqual(seed.data.hexEncodedString(), "2c2b317796b25870b26ada21fd962f3c17cae29825704fbc1c89784546047372ffb01ed32649095df4188832c7c1cbfff226327f4f000cd35a7f3373034396d8")
    }
    
    // MARK: - Generate 32 -
    
    func testGenerate32ByteEntropy() throws {
        let entropy = try XCTUnwrap(Seed32.generate())
    ...
    func testGenerate24Words() throws {
        let entropy = try XCTUnwrap(Seed32.generate())
        let phrase = entropy.mnemonic
        XCTAssertEqual(phrase.words.count, 24)
    }
    
    func testGenerateSeedFrom24Words() {
        let entropy = Seed32(base58: "GNptCwCgxijdNumSpQW49Mc3r6JvfdTQz7qPn14K5oFi")!
        let seed = Derive.seedUsingBIP39(phrase: entropy.mnemonic.words)
        XCTAssertEqual(seed.data.hexEncodedString(), "8ea6cc7058e9937640d8649808a32d3ed542ff12256bfb7c3fe01af0122ed3dc18295005197002e4c6216071bfcaac7362c9f4dfacdb8d93d27a83f637cd5efd")
    }
    
    // MARK: - Curve -
    
    func testOffCurve() {
        let offCurvePublicKeys = PublicKey.offCurveKeys()
...
private extension PublicKey {
    static func offCurveKeys() -> [PublicKey] {
        [
