...
class ShortVecTests: XCTestCase {
    ...
    func testEncode() {
        ...
        XCTAssertEqual(encoded.bytes, [0])

        encoded = ShortVec.encodeLength(5)
        XCTAssertEqual(encoded.bytes, [5])

        encoded = ShortVec.encodeLength(0x7f)
        XCTAssertEqual(encoded.bytes, [0x7f])

        encoded = ShortVec.encodeLength(0x80)
        XCTAssertEqual(encoded.bytes, [0x80, 0x01])

        encoded = ShortVec.encodeLength(0xff)
        XCTAssertEqual(encoded.bytes, [0xff, 0x01])

        encoded = ShortVec.encodeLength(0x100)
        XCTAssertEqual(encoded.bytes, [0x80, 0x02])

        encoded = ShortVec.encodeLength(0x7fff)
        XCTAssertEqual(encoded.bytes, [0xff, 0xff, 0x01])
    }
    ...
    func testValidity() {
        for i in 0..<UInt8.max {
            ...
            XCTAssertEqual(i, Byte(actual.length))
        }
    }
    ...
    func testCrossImplementation() {
        [
            ...
            XCTAssertEqual($0.encoded.count, output.count)
            XCTAssertEqual($0.encoded, output.bytes)
        }
    }
}
